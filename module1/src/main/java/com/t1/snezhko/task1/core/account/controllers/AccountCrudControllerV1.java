...
public class AccountCrudControllerV1 {
    @Autowired
    ...
    public ResponseEntity<Void> unlockAccountById(@PathVariable("id") String accountId) {
        UUID id = UUID.fromString(accountId);
        AccountStatus status = accountCrudService.getAccountById(id).getStatus();
        if (status.equals(AccountStatus.ARRESTED)) {
            return ResponseEntity.status(FORBIDDEN).build();
        }

        accountCrudService.updateStatus(id, AccountStatus.OPEN);
